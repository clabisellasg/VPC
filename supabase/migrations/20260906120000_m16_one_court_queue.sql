-- M16: one durable, organizer-operated court queue per event.
create table private.court_queue_operation_receipts (
...
revoke all on private.court_queue_operation_receipts from public, anon, authenticated;

create index court_queue_event_waiting_idx
  on public.court_queue_entries(event_id, queue_position, id)
...
with match_rows as (
  ...
  where d.event_id=p_event_id and d.deleted_at is null and m.deleted_at is null
...
create function public.get_court_queue_context(p_event_id uuid)
returns jsonb language plpgsql stable security definer set search_path='' as $$
declare response jsonb;
begin
  response:=private.court_queue_context(p_event_id);
  if response is null then raise exception 'Event not found' using errcode='P0002'; end if;
  return response;
end $$;
...
declare
  ...
  v_expected_version bigint; stamp timestamptz:=clock_timestamp();
...
begin
  ...
  if jsonb_typeof(p_payload)<>'object' or exists(
    select 1 from jsonb_object_keys(p_payload) key where not (key=any(allowed_keys))) then
  ...
  select * into prior from private.court_queue_operation_receipts r where r.operation_id=v_operation_id;
  if found then
  ...
  perform 1 from public.events e where e.id=v_event_id and e.deleted_at is null for update;
  ...
  if v_action='reconcile' then
    ...
    where q.event_id=v_event_id and q.deleted_at is null and exists(
...
revoke all on function public.get_court_queue_context(uuid),public.apply_court_queue_operation(jsonb) from public,anon,authenticated;
grant execute on function public.get_court_queue_context(uuid) to anon,authenticated;
grant execute on function public.apply_court_queue_operation(jsonb) to authenticated;

create function private.guard_one_current_court_match()
returns trigger language plpgsql set search_path='' as $$
declare event_scope uuid;
begin
  if new.status<>'inProgress' or new.deleted_at is not null then return new; end if;
  select d.event_id into event_scope from public.event_divisions d where d.id=new.division_id;
  if exists(select 1 from public.matches m join public.event_divisions d on d.id=m.division_id
    where d.event_id=event_scope and m.id<>new.id and m.status='inProgress' and m.deleted_at is null) then
    raise exception 'Only one match may be Now Playing' using errcode='23505'; end if;
  return new;
end $$;
create trigger matches_one_current_court_guard before insert or update of status,deleted_at,division_id
on public.matches for each row execute function private.guard_one_current_court_match();
