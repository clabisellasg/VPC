-- Focused correction regression: all fixtures roll back; run after RPC repair.
  ...
  begin
    ...
    for played in select value from jsonb_array_elements(response->'bracket'->'matches') loop
      ...
      foreach k in array array['start','result'] loop
        ...
        if match_key='se/r2/m1' and k='start' then
          begin
            perform public.apply_single_elimination_operation(payload||jsonb_build_object(
              'operation_id',md5('VPC M13 started downstream rejection')::uuid,'action','correct',
              'match_key','se/r1/m1','expected_version',(response->'bracket'->>'version')::bigint,
              'score_one',11,'score_two',9,'reason','VPC synthetic downstream started'));
            raise exception 'In-progress downstream correction accepted';
      ...
      if match_key='se/r1/m1' then
        ...
        response:=public.apply_single_elimination_operation(payload);
        if not exists(select 1 from public.match_result_revisions where operation_id=md5('VPC M13 correction')::uuid
            and previous_result->>'winner_team_id'=md5('VPC M13 team 4:1')::uuid::text) then raise exception 'Missing original immutable revision';end if;
        if (select side_one_team_id from public.matches where id=md5('VPC M13 match 4:se/r2/m1')::uuid)
          is distinct from md5('VPC M13 team 4:4')::uuid then raise exception 'Corrected winner did not advance';end if;
        if public.apply_single_elimination_operation(payload)<>response then raise exception 'Correction replay failed';end if;
        begin perform public.apply_single_elimination_operation(payload||jsonb_build_object('reason','different'));
          raise exception 'Changed correction replay accepted';exception when unique_violation then null;end;
        begin perform public.apply_single_elimination_operation(payload||jsonb_build_object('operation_id',md5('VPC M13 stale correction')::uuid));
          raise exception 'Stale correction accepted';exception when serialization_failure then null;end;
        perform set_config('request.jwt.claim.sub',member_actor::text,true);
        begin perform public.apply_single_elimination_operation(payload);
          raise exception 'Member correction accepted';exception when insufficient_privilege then null;end;
        perform set_config('request.jwt.claim.sub','',true);
        begin perform public.apply_single_elimination_operation(payload);
          raise exception 'Anonymous correction accepted';exception when insufficient_privilege then null;end;
        perform set_config('request.jwt.claim.sub',actor::text,true);
      end if;
    end loop;
    ...
    payload:=payload||jsonb_build_object('operation_id',md5('VPC M13 final correction')::uuid,'action','correct',
      'expected_version',(response->'bracket'->>'version')::bigint,'score_one',9,'score_two',11,'reason','VPC synthetic final correction',
      'placement_ids',jsonb_build_object('1',md5('VPC M13 corrected champion')::uuid,'2',md5('VPC M13 corrected runner up')::uuid));
    response:=public.apply_single_elimination_operation(payload);
    if (select team_id from public.division_placements where division_id=did and position=1 and deleted_at is null)
      is distinct from md5('VPC M13 team 4:2')::uuid
      or (select team_id from public.division_placements where division_id=did and position=2 and deleted_at is null)
      is distinct from md5('VPC M13 team 4:4')::uuid then raise exception 'Final correction placements incorrect';end if;
    if (select count(*) from public.match_result_revisions r join public.matches m on m.id=r.match_id where m.division_id=did)<>2
      then raise exception 'Correction audit history lost';end if;
    if public.apply_single_elimination_operation(payload)<>response then raise exception 'Final correction replay failed';end if;
